// === SPLASH AMARELO GLOBAL (compatível com todas as funções) ===
(function() {
  ...
  // Exibir splash na abertura do site
  ...
  // Substituir transição de página
  const originalHandleTransition = window.handlePageTransition;
  window.handlePageTransition = function(url) {
    showSplash(500);
    setTimeout(() => {
      if (typeof originalHandleTransition === 'function') {
        originalHandleTransition(url);
      } else {
  ...
  // Envolver modal (caso funções existam após DOM pronto)
